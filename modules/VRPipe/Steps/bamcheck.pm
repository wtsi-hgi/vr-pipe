    ...
    method stats_from_bamcheck (ClassName|Object $self: Str $cmd_line!, Bool $targeted, Bool $dups) {
        ...
        if ($check_file->s) {
            ...
            unless ($hash_key_prefix eq 'rmdup_') {
                ...
                my $bases_dup_fraction = $parser->bases_mapped ? $bases_duplicated / $parser->bases_mapped : 0;
                my $bases_dup_estimate = int($parser->bases_mapped_cigar * $bases_dup_fraction);
                $new_meta->{ $hash_key_prefix . 'rmdup_bases_mapped' } = $parser->bases_mapped_cigar - $bases_dup_estimate;
