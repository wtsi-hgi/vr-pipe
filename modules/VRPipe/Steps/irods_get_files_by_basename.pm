            ...
            foreach my $file (@{ $self->inputs->{basenames} }) {
                ...
                if (!-s $out_abs_path) {
                    ...
                    if ($irods_abs_path) {
                        ...
                        $self->dispatch_vrpipecode(qq[use VRPipe::Steps::irods_get_files_by_basename; VRPipe::Steps::irods_get_files_by_basename->get_file_by_basename(basename => q[$basename], dest => q[$out_abs_path], zone => q[$zone], iget => q[$iget], iquest => q[$iquest], ichksum => q[$ichksum], path_regex => q[${path_regex}]$extra);], $req);
                    }
                }
                else {
