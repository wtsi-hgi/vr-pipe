...
class VRPipe::Steps::vrtrack_auto_qc_hgi extends VRPipe::Steps::vrtrack_update {
    use VRPipe::Parser;
    ...
    around options_definition {
        return {
            ...
            auto_qc_max_ic_above_median => VRPipe::StepOption->create(
                description   => 'Maximum indels per cycle, factor above median',
                optional      => 1,
                default_value => '8'
            ),
            auto_qc_indel_percentage_deviation => VRPipe::StepOption->create(
                description   => 'Maximum allowed percentage deviation of indel peaks per cycle',
                optional      => 1,
                default_value => '3'
            ),
            auto_qc_qual_contig_cycle_dropoff => VRPipe::StepOption->create(
                description   => 'Maximum number of contiguous cycles of qual dropoff permitted',
                optional      => 1,
                default_value => '9'
            ),
    ...
    method auto_qc (ClassName|Object $self: Str :$db!, Str|File :$bam!, Str|File :$bamcheck!, Str :$lane!, Str :$auto_qc_gtype_regex?, Num :$auto_qc_mapped_base_percentage?, Num :$auto_qc_duplicate_read_percentage?, Num :$auto_qc_mapped_reads_properly_paired_percentage?, Num :$auto_qc_error_rate?, Num :$auto_qc_insert_peak_window?, Num :$auto_qc_insert_peak_reads?, Num :$auto_qc_max_ins_to_del_ratio?, Num :$auto_qc_min_ins_to_del_ratio?, Num :$auto_qc_overlapping_base_duplicate_percent?,  Num :$auto_qc_max_ic_above_median?, Num :$auto_qc_indel_percentage_deviation?, Num :$auto_qc_qual_contig_cycle_dropoff? ) {
        ...
        # HGI #
        #######
        if (defined $auto_qc_indel_percentage_deviation) {
            $status = 1;
            $reason = "All indels per cycle deviate less then ${auto_qc_indel_percentage_deviation} percent from baseline.";

            if ($bc->fwd_percent_insertions_above_baseline() > $auto_qc_indel_percentage_deviation) {
                $status = 0;
                $reason = "Fwd insertions deviate more than ${auto_qc_indel_percentage_deviation} percent above the baseline.";
	        push @qc_status, { test => 'Indel Deviation', status => $status, reason => $reason };
            }

            if ($bc->fwd_percent_insertions_below_baseline() > $auto_qc_indel_percentage_deviation) {
                $status = 0;
                $reason = "Fwd insertions deviate more than ${auto_qc_indel_percentage_deviation} percent below the baseline.";
                push @qc_status, { test => 'Indel Deviation', status => $status, reason => $reason };
            }

            if ($bc->fwd_percent_deletions_above_baseline() > $auto_qc_indel_percentage_deviation) {
                $status = 0;
                $reason = "Fwd deletions deviate more than ${auto_qc_indel_percentage_deviation} percent above the baseline.";
                push @qc_status, { test => 'Indel Deviation', status => $status, reason => $reason };
            }

            if ($bc->fwd_percent_deletions_below_baseline() > $auto_qc_indel_percentage_deviation) {
                $status = 0;
                $reason = "Fwd deletions deviate more than ${auto_qc_indel_percentage_deviation} percent below the baseline.";
                push @qc_status, { test => 'Indel Deviation', status => $status, reason => $reason };
            }

            if ($bc->rev_percent_insertions_above_baseline() > $auto_qc_indel_percentage_deviation) {
                $status = 0;
                $reason = "Rev insertions deviate more than ${auto_qc_indel_percentage_deviation} percent above the baseline.";
                push @qc_status, { test => 'Indel Deviation', status => $status, reason => $reason };
            }

            if ($bc->rev_percent_insertions_below_baseline() > $auto_qc_indel_percentage_deviation) {
                $status = 0;
                $reason = "Rev insertions deviate more than ${auto_qc_indel_percentage_deviation} percent below the baseline.";
                push @qc_status, { test => 'Indel Deviation', status => $status, reason => $reason };
            }

            if ($bc->rev_percent_deletions_above_baseline() > $auto_qc_indel_percentage_deviation) {
                $status = 0;
                $reason = "Rev deletions deviate more than ${auto_qc_indel_percentage_deviation} percent above the baseline.";
                push @qc_status, { test => 'Indel Deviation', status => $status, reason => $reason };
            }

            if ($bc->rev_percent_deletions_below_baseline() > $auto_qc_indel_percentage_deviation) {
                $status = 0;
                $reason = "Rev deletions deviate more than ${auto_qc_indel_percentage_deviation} percent below the baseline.";
                push @qc_status, { test => 'Indel Deviation', status => $status, reason => $reason };
            }
        ...
        if (defined $auto_qc_qual_contig_cycle_dropoff) {
            $status = 1;
            $reason = "Quality drops off no more than ${auto_qc_qual_contig_cycle_dropoff} contigious times.";

            if ($bc->contiguous_cycle_dropoff_count() > $auto_qc_qual_contig_cycle_dropoff) {
                $status = 0;
                $reason = "Quality drops off more than ${auto_qc_qual_contig_cycle_dropoff} contigious times.";
            }
