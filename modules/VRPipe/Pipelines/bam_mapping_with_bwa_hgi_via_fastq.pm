...
class VRPipe::Pipelines::bam_mapping_with_bwa_hgi_via_fastq with VRPipe::PipelineRole {
    method name {
        return 'bam_mapping_with_bwa_hgi_via_fastq';
    }
    ...
    method step_names {
        (
            ...
            'bwa_aln_fastq',         #5->7
            'bwa_sam',               #6->8
            'sam_to_fixed_ns_bam',   #7->9
            'bam_merge_lane_splits', #10
            'bam_merge_align',       #8->11
            'bam_sort',              #9->12
            'bam_calculate_bq',      #10->13
            'bam_mark_duplicates',   #11->14
        );
    }
    ...
    method adaptor_definitions {
        (
            ...
            { from_step => 9, to_step => 10, from_key => 'fixed_bam_files', to_key => 'aligned_bam_files' },
            { from_step => 11, to_step => 12, from_key => 'merge_align_bam_files', to_key => 'bam_files' },
            { from_step => 12, to_step => 13, from_key => 'sorted_bam_files', to_key => 'bam_files' },
            { from_step => 13, to_step => 14, from_key => 'bq_bam_files', to_key => 'bam_files' },
        );
    }
    ...
    method behaviour_definitions {
        (
            ...
            { after_step => 9, behaviour => 'delete_outputs', act_on_steps => [6, 7, 8], regulated_by => 'cleanup', default_regulation => 1 },
            { after_step => 10, behaviour => 'delete_outputs', act_on_steps => [9], regulated_by => 'cleanup', default_regulation => 1 },
            { after_step => 11, behaviour => 'delete_outputs', act_on_steps => [4, 10], regulated_by => 'cleanup', default_regulation => 1 },
